...
def cr_parser(buf) -> Iterator[tuple[bytes, int]]:
    """Carriage Return (CR) Parser.

    Generator that returns all bytes until a carriage return is found.
    Additionally returns the index to the next character to parse (can
    be more than the buffer length).
    """
    pos = 0
    while pos < len(buf):
        # find the index of the first \r byte which is followed by an \n byte
        end = buf.find(b"\r\n", pos)
